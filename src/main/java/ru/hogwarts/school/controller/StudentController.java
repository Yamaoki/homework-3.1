...
public final class StudentController {
    ...
    @GetMapping
    public ResponseEntity<Student> findStudents(@RequestParam(required = false) Long age) {
        if (age > 0) {
            return ResponseEntity.ok(studentService.getStudentById(age));
        }
